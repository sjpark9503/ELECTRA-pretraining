...
class ElectraForPreTraining(ElectraPreTrainedModel):
    ...
    # @add_start_docstrings_to_callable(ELECTRA_INPUTS_DOCSTRING)
    # r@eplace_return_docstrings(output_type=ElectraForPretrainingOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids=None,
        attention_mask=None,
...
# MODIFIED #
############
class ElectraForPreTrainingWithGenerator(ElectraPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.lamb = 50
        self.generator_scaling = 1/4
        self.discriminator = ElectraForPreTraining(config)
        self.word_probs = nn.Softmax()
    ...
    def forward(
        ...
        **kwargs
        ...
        #return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        return_dict=False
        G_output = self.generator(input_ids,
            attention_mask,
            token_type_ids,
            position_ids,
            head_mask,
        ...
        size = G_output[-1].size()
        sample = torch.multinomial(self.word_probs(G_output[-1]).reshape(-1,size[2]),1).reshape(size[0],size[1])
        D_input_ids, D_labels = self.replace_token(input_ids.clone(),org_input_ids,sample)
        D_output = self.discriminator(D_input_ids,
            attention_mask,
            token_type_ids,
            position_ids,
            head_mask,
        ...
        if isinstance(D_output,tuple) and isinstance(G_output,tuple):
            loss = G_output[0] + self.lamb*D_output[0]
        return (loss,G_output,D_output,) if loss is not None else (G_output,D_output,)

    def replace_token(self,input_ids,org_input_ids,sample):
        mask_indices = ~torch.eq(input_ids,org_input_ids)
        input_ids[mask_indices]=sample[mask_indices]
        labels = torch.eq(org_input_ids, input_ids)
