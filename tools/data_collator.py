...
from tools.gen import replace_sentence
...
class DataCollatorForLanguageModeling:
    ...
    def __call__(self, examples: List[Union[torch.Tensor, Dict[str, torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        if self.mode == "mlm":
            inputs, labels, org_inputs = self.mask_tokens(batch)
            return {"input_ids": inputs, "labels": labels, "org_input_ids":org_inputs}
        elif self.mode == "wordnet:
            inputs, labels, org_inputs = self.wordnet_words(batch)
            return {'input_ids': inputs, "labels": labels, "org_inputs_ids": org_input}
        elif self.mode == "random":
            inputs, labels, org_inputs = self.random_words(batch)
            return {'input_ids': inputs, "labels": labels, "org_inputs_ids": org_input}
        else:
    ...
    def mask_tokens(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        return inputs, labels, org_inputs

    def wordnet_words(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Prepare new words inputs/labels: if word doesn't exist - original else replaced word
        """
    ...
    def random_words(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
        """
